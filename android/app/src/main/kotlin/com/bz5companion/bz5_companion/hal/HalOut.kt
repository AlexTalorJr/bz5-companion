...
class JournalHalOut(private val ctx: Context) : HalOut {
    ...
    companion object {
        ...
        /**
         * v0.1.94+193 — ПАРА ЯЧЕЕК ПИШЕТСЯ ВСТРЕЧНО.
         *
         * Порог по имени решил задачу бюджета журнала и создал новую,
         * которую поле показало 04.08. Оба ячейковых сигнала придушены по
         * 10 с КАЖДЫЙ и НЕЗАВИСИМО, а фаза у них своя. Замер по экспорту
         * фоновой поездки: медиана расстояния от строки до парной ей — 3 с,
         * в пределах секунды всего 19 пар из 113. При скорости изменения
         * напряжений p90 около 11 мВ/с это до трёх десятков милливольт
         * выдуманной разности, и в пяти строках из девятнадцати она
         * перевалила знак: разброс вышел отрицательным.
         *
         * Разность имеет смысл только у пары ОДНОГО мгновения. В живом
         * потоке она такая сама: там 48 Гц. В журнале одномоментность
         * убивает придушивание, поэтому её надо возвращать здесь, а не
         * угадывать правилами при чтении — угадывание проверено зеркалом и
         * не работает ни при каком окне, кроме жёсткой секунды, да и та
         * оставляет треть строк.
         *
         * Механика: когда пишется одно имя пары, второе становится ДОЛЖНЫМ,
         * то есть проходит на своём следующем кадре — через десятки
         * миллисекунд при 48 Гц. Строк не прибавляется: каждое имя
         * по-прежнему пишется раз в свой период.
         *
         * Тонкость, из-за которой нужна `PAIR_GRACE_MS`: без неё пара
         * зациклилась бы. Запись первого делает должным второй, запись
         * второго — снова первый, и оба писали бы на каждом кадре. Поэтому
         * второе имя подтягивается ТОЛЬКО вперёд: если оно писалось совсем
         * недавно, его не трогают.
         */
        val PAIRED_WITH: Map<String, String> = mapOf(
            "cell_v_lowest" to "cell_v_highest",
            "cell_v_highest" to "cell_v_lowest",
        )

        /** Насколько недавняя запись парного имени считается свежей. */
        const val PAIR_GRACE_MS: Long = 250L
    ...
    private fun append(batch: List<Map<String, Any?>>) {
        ...
        for (e in batch) {
            ...
            lastPerName[name] = ts
            // Парное имя становится должным — но только подтягиваясь
            // вперёд, иначе пара зациклится (см. PAIRED_WITH).
            val partner = PAIRED_WITH[name]
            if (partner != null) {
                val pPrev = lastPerName[partner]
                if (pPrev == null || ts - pPrev > PAIR_GRACE_MS) {
