 ...
 *
 * ── ПОЛЕ 30.07: SAF МЁРТВ, СЕТЬ ЖИВА ─────────────────────────────
 *
 * Визит ответил на всё, ради чего делался, и порядок путей изменился.
 *
 *   • DocumentsUI на прошивке НЕТ ВОВСЕ. Оба интента дерева —
 *     `ACTION_OPEN_DOCUMENT_TREE` и `createOpenDocumentTreeIntent()` —
 *     дают `ActivityNotFoundException`. Путь A1a мёртв; оставлен
 *     пробой, потому что стоит он два вызова, а прошивки бывают разные.
 *   • §B РАБОТАЕТ: голова сходила в GitHub и прочитала
 *     `build-254 · bz5-companion-0.1.78.254.apk · 30171855 B`. Файл
 *     можно получить БЕЗ хранилища и без выбора файла, прямо в кэш,
 *     откуда его читает провайдер. Первая стена обходится целиком —
 *     поэтому сеть теперь ПЕРВЫЙ путь, а не последний.
 *   • Из девяти дверей живы три: `APPLICATION_DETAILS_SETTINGS`,
 *     `MANAGE_APPLICATIONS_SETTINGS`, `GET_CONTENT` (по одному
 *     резолверу). Остальные шесть — ноль.
 *   • `targetSdk` ИЗМЕРЕН: 35. Хендофф был прав, а моя поправка в +177
 *     — нет: я опроверг верное число по неверно прочитанному логу
 *     сборки (установка Android SDK Platform 33 относится к
 *     compileSdk модуля, а не к нашему targetSdk). Структурный потолок
 *     пути A1c этим подтверждён окончательно.
 *   • Установщик отзывается: `com.android.packageinstaller/…InstallStart`,
 *     `can_request_installs=false`. Ответ установщика по-прежнему
 *     НЕИЗВЕСТЕН — до него не дошло ни разу, файла не было.
 *
 * Порядок путей к файлу теперь: §B (сеть) → `GET_CONTENT` и
 * «поделиться» → SAF-дерево (проба) → File-API (проба).
 *
 *   A1a. SAF-ДЕРЕВО — исторически первым. `ACTION_OPEN_DOCUMENT_TREE` и
 ...
 *        причине.
 *
 *        ИЗМЕРЕНО ПОЛЕМ 30.07: `targetSdk` = 35. Поправка +177, где я
 *        объявил это число сомнительным, СНЯТА — она была ошибкой
 *        второго рода: верное значение опровергнуто по неверно
 *        прочитанной улике (установка Android SDK Platform 33 в логе
 *        сборки относится к compileSdk модуля, а не к нашему
 *        targetSdk). Ключ `target_sdk` в пробе затем и добавлялся.
 *
...
object ApkInstall {
    ...
    /**
     * Ещё один системный выбор файла — через `ACTION_GET_CONTENT`.
     *
     * v0.1.79+178, и это прямое следствие поля 30.07: из девяти дверей
     * на прошивке живы три, и `GET_CONTENT` одна из них (один
     * резолвер). Действие ДРУГОЕ, чем `ACTION_OPEN_DOCUMENT`, который
     * перехвачен галереей BYD, — значит и обработчик может быть другим.
     * Проверить это стоит одного вызова, а цена незаданного вопроса —
     * визит к машине.
     *
     * Ответ приходит тем же кодом REQ_PICK: дальше пути сходятся, файл
     * в обоих случаях стажируется одинаково.
     */
     ...
     *
     *  ПОЛЕ 30.07: на этой прошивке перехвачен галереей BYD и
     *  возвращает cancelled. Оставлен пробой. */
    ...
    fun openTree(activity: Activity): Map<String, Any?> {
        ...
        // Причина обязана быть НАСТОЯЩЕЙ. В +176 её здесь не было, и
        // Dart подставлял «cancelled» — то есть журнал сообщал, что
        // владелец отменил выбор, когда на прошивке просто нет активити.
        // Поле 30.07: из-за этой строки кнопка выглядела нерабочей,
        // хотя отрабатывала штатно и дважды писала ActivityNotFound.
        out["ok"] = false
