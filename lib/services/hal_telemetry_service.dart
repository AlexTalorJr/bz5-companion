...
class HalTelemetryService extends ChangeNotifier
  ...
  void _adoptJoinedTail(_HalJoinTail j) {
    ...
    // v0.1.93+192 — ЯКОРЯ БЕРУТСЯ ИЗ КЛАСТЕРА, А НЕ ДОСТРАИВАЮТСЯ.
    //
    // Здесь стояло `??=` с обоснованием «живой старт мог успеть
    // защёлкнуть свои якоря из свежих кадров, и они точнее прошлых».
    // ПОЛЕ ОПРОВЕРГЛО ЭТО ЗА ОДИН ЗАЕЗД. Поездка 03.08 12:09:56:
    //
    //   start_odometer в строке   6700.8   (защёлкнут живым стартом)
    //   первая строка кластера    6699.8   (настоящее начало движения)
    //   distance_km                  2.5
    //   6700.8 → 6702.3               1.5
    //
    // Строка вышла внутренне противоречивой: два числа про одну поездку.
    // Ошибка была в самом слове «точнее»: живое значение не точнее, оно
    // ПОЗЖЕ, а у поездки с отодвинутым назад началом якоря обязаны
    // соответствовать этому началу. Кластер здесь — единственный
    // источник правды, и он побеждает, когда значение у него есть.
    _halTripStartOdo = j.startOdo ?? _halTripStartOdo;
    _halTripStartSoc = j.startSoc ?? _halTripStartSoc;
