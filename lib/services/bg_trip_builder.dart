...
class BgTripBuilder {
  ...
  static Future<bool> _buildOne(
    ...
    final tripId = await db.insertTripWithStampedSamples(
      ...
      // читает `extra.source == "hal_bg"` и исключает такую поездку из
      // инварианта #8 (разрыв одометра): она по построению начинается там,
      // где предыдущей поездки нет, и это не аномалия, а её починка.
      // Имена ключей — контракт, см. CLIENT_API.md §3.1.
