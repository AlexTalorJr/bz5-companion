...
class ExportService {
  ...
  Future<_BuildResult> _buildZip({
    ...
    // 31.07 доказал именно запись).
    //
    // v0.1.88+187 — ЗДЕСЬ СТОЯЛО ОБЕЩАНИЕ, ЧТО ОТКАЗ НИЧЕГО НЕ СТОИТ:
    // мол, архив с меткой времени уже на диске, и импорт доберётся до
    // него перечислением каталога. ЭТО БЫЛО НЕВЕРНО, и неверно ровно
    // тем способом, который стоил поля 02.08: `findArchive` возвращал
    // файл по `exists()` и до перечисления не доходил НИКОГДА. Значит
    // чужой нечитаемый файл под постоянным именем заслонял собой свежий
    // архив, написанный нами же и прекрасно читаемый. Текст описывал
    // свойство, которого у кода не было.
    //
    // Свойство теперь есть — выбор идёт по читаемости, — но обещание
    // отсюда убрано: пусть его держит тот код, который его выполняет.
    // Отказ перестал быть немым: строка уходит в журнал приложения, а
    // он уезжает диаг-дампом.
    // ── КОПИЮ ПОД ПОСТОЯННЫМ ИМЕНЕМ БОЛЬШЕ НЕ ПИШЕМ ──
    //
    // v0.1.94+193. На этом ГУ имя мертво по устройству прошивки: файл,
    // созданный прежней установкой, принадлежит прежнему uid, дописать и
    // перезаписать его нельзя, а прочитать — тем более. Путаницы это
    // принесло дважды, и второй раз дорого.
    //
    // Поле 04.08: жалоба «экспорт негоден» указывала на
    // `bz5_export_latest.zip` с 4 585 082 байтами при ожидаемых 6 548 267 —
    // и это байт в байт СТАРЫЙ файл от 03.08. То есть сверка записи
    // проверяла файл, записать который мы не смогли вовсе, и жаловалась на
    // чужую длину. Свежий архив со штампом времени при этом был цел.
    //
    // Перестаём писать — и жалоба исчезает вместе с причиной. УЗНАВАТЬ имя
    // в перечислении при восстановлении мы продолжаем (ImportService
    // держит его в списке): перестать узнавать — это смена контракта, а не
    // уборка. Единственное постоянное имя, которое мы создаём сами, —
    // `kStagedName`.
    ...
    // Сверяется ТОЛЬКО то, что мы писали. Прежний список включал копию
    // под постоянным именем, и её отказ выглядел как обрезанный экспорт.
