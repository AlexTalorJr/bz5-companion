  ...
  MigrationStrategy get migration => MigrationStrategy(
        ...
        onUpgrade: (m, from, to) async {
          ...
          if (from < 19) {
            ...
            // ── И АГРЕГАТ ФОНОВЫХ ПОЕЗДОК (v0.1.95+194) ──
            //
            // Шаг 19 приехал в +193 и чистил только снапшоты — то есть
            // оставлял на экране ровно то число, против которого писался.
            // Вторая команда дописана в ТОТ ЖЕ шаг: у владельца база ещё
            // восемнадцатая, шаг не исполнялся, и заводить ради этого
            // двадцатый значило бы плодить ступень без нужды. Разброс
            // поездки лежит в ДРУГОЙ таблице, и именно его показывает
            // карточка разбаланса в Трендах: у #155 там 190 мВ при живой
            // поездке того же дня в 4…9.
            //
            // Условие шире, чем «отрицательный», и это не небрежность.
            // Агрегат считался МАКСИМУМОМ по независимо перенесённым
            // значениям, то есть по построению отбирал самый большой
            // рассинхрон из всех. Отдельный снапшот мог случайно попасть в
            // одномоментную пару и быть верным; максимум — не мог. Он лжив
            // у КАЖДОЙ фоновой поездки, построенной прежним кодом, поэтому
            // и снимается у всех разом.
            //
            // Живых поездок не касается: там значение накапливалось по
            // паре, проверенной окном, на потоке в 48 Гц.
            //
            // `IS NOT NULL` в условии — ради счётчика: без него повторный
            // прогон снова «правил» бы те же строки, записывая NULL поверх
            // NULL, и идемпотентность нельзя было бы показать числом.
            await customStatement(
              'UPDATE trips SET max_cell_spread_mv = NULL '
              "WHERE source = 'hal_bg' AND max_cell_spread_mv IS NOT NULL",
            );
            debugPrint('DB migrate: v19 — выдуманные тройки ячеек и агрегаты '
                'фоновых поездок вычищены');
          }
