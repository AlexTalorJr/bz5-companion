...
|------------|---------------------------|
| 0.1.29+18  | P1.5 Cloud restore (strategy A+restore per HANDOFF; head-unit reinstall is the primary use case — Toyota DiLink не поддерживает in-place apk update, только uninstall+reinstall, что стирает Drift+secureStorage+SharedPrefs). CloudSyncService получил три новых публичных метода: `probeRestoreToken(t)` — валидация candidate client_token через `GET /v1/data/trips?limit=1` (возвращает device_id из префикса token'а); `startRestore({oldClientToken})` — fire-and-forget pull: swap token + secureStorage write → отключение periodic push на время → `GET /v1/data/trips` (limit=100) и `GET /v1/data/snapshots` (limit=200) с pagination через `next_cursor`, dedup `(started_at, distance_km)` для trips и `captured_at` для snapshots, билд `Map<int,int> serverClientTripId → newLocalId` для rewrite snapshot.tripId, finally — `_cursorTrip/_cursorSnapshot = max(local id)` (предотвращает re-push restored rows под restored identity), `_enabled=true` на success (чтобы periodic sync стартовал после restore-from-disconnected); `cancelRestore()` — флаг для loop, проверяется между pages. Новый `_getJson(path, query)` helper (~70 LOC, fокус: 401 → _Transient/AuthException, 408/429/5xx → backoff из _retryBackoff). Новые enum `CloudRestoreStatus` (idle/preflight/fetching/done/cancelled/error) + value class `CloudRestoreProgress`. SharedPrefs: `_kLastRestoreAt`/`_kLastRestoreError` (status non-persistent — на app start всегда idle). Settings UI: новая кнопка "Restore from cloud" появляется **в двух местах** — (1) в disconnected-state как альтернатива Setup, рядом с "Set up cloud backup" (это ключевой head-unit reinstall path — нужен только один токен от Друга 2, без round-trip за setup_token); (2) в registered-state в Wrap'е кнопок между Force resync и Disconnect (для редкого случая merge-history между девайсами). 3-фазный диалог: token → probe loader → confirmation с warning про local-id collision → progress dialog с `Consumer<CloudSyncService>` + Cancel/Close. После успеха в Settings — `BridgeDiagService.refreshTokenFromSharedStorage()` (Plane A разделяет тот же secureStorage token; та же логика что и после Setup). Inline restore status card отображается и в disconnected-, и в registered-секции Cloud backup body. Sync / Force resync / Disconnect disabled когда isRestoring. Scope этой итерации: только trips + snapshots; sweeps/livelogs — отдельный патч если понадобится. Touched: cloud_sync_service.dart (+~440 LOC), settings.dart (+~290 LOC, добавлен `import 'dart:async'` для unawaited). Protected files (database.dart/connection.dart/elm327_ble.dart) **не тронуты** — restore делает raw `_db.into(_db.trips)` / `_db.select(_db.snapshots)` запросы через standard Drift external API, добавлен только `import 'package:drift/drift.dart' show Value`. cloud_sync `_readAppVersion` → 0.1.29+18. |
