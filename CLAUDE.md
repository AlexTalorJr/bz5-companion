...
|------------|---------------------------|
| 0.1.29+20  | P5.1 Trip finalize fix (client-side). До этого `_syncTrips` использовал `t.id > _cursorTrip` для gating push'а — это работало ровно один раз на trip при его insert'е (с `endedAt=NULL` + все aggregates NULL); после `endTrip()` в Drift trip row обновлялся, но cursor уже сдвинут → finalize-данные на сервер никогда не уходили. Друг 2 подтвердил симптом: trip id=1 от утра 2026-05-25 лежал в Postgres с ended_at=NULL. Этот патч идёт в паре с серверным UPSERT (его коммит, §P5.2 ROADMAP); ни один из двух fix'ов сам по себе не решит проблему. Изменения в CloudSyncService: новый SharedPrefs key `cloud_sync_pushed_trip_ids` (JSON int list), новое in-memory поле `Set<int> _pushedTripIds`. Push gating: `where t.endedAt != null && !_pushedTripIds.contains(t.id)` → POST → `_pushedTripIds.addAll(batch.ids)` → persist. Server-side UPSERT (uniq key device_id+client_trip_id) absorbs accidental re-push, что делает crash между POST и persist безопасным. Открытые trips (endedAt=null) intentionally not pushed — owner не запрашивал live monitoring во время drive'а. Migration на first-run +20 (когда `_kPushedTripIds` отсутствует но `_cursorTrip > 0` — значит был upgrade с +17/+18/+19): seed set'а ids всех locally-closed trips с `id <= _cursorTrip`. Открытые trips, ошибочно push'нутые +19 с NULL aggregates, останутся вне set'а → следующий close + syncOnce → re-push с full aggregates → server UPSERT исправит. `_cursorTrip` retained для stats (max id seen), но больше не gating; `forceFullResync()` дополнительно сбрасывает `_pushedTripIds` (это и есть основная семантика "replay everything"). Restore path: closed trips добавляются в set (они уже на сервере), open trips остаются вне (они могут быть теми half-finalized что мы хотим re-push'нуть). `_recomputeStats` для trips переписан с `maxTrip - _cursorTrip` на `count(closed && not pushed)` — точно отражает actionable pending. Snapshots/sweeps/livelogs не тронуты (append-only, cursor стратегия валидна). Touched: cloud_sync_service.dart (~85 LOC net add). pubspec + _kDiagVersion + _readAppVersion → 0.1.29+20. Database.dart не тронут (no schema change, использует existing `getAllTrips()`). |
