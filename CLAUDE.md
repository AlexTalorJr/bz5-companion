...
3. **Установка APK ведёт себя по-разному на двух устройствах**:
   - **Phone** (Android, разработческое устройство): обновление инсталлируется **поверх**, данные сохраняются (Drift / secureStorage / SharedPrefs). In-place update работает.
   - **Head Unit** (Toyota DiLink в машине): **нельзя обновить APK поверх**, можно только uninstall + install. Это **стирает Drift / secureStorage / SharedPrefs**. Все state-resilient функции (cloud sync, BridgeDiag, Restore-from-cloud) спроектированы вокруг этого ограничения — head unit рассматривается как ephemeral install с restore-from-cloud workflow'ом. При планировании deploy'а пути для head unit всегда предполагай uninstall+install последовательность, **никогда** не пиши "in-place upgrade" применительно к head unit. Подпись APK одинакова на всех билдах (keystore из GitHub Actions secrets) — но это влияет только на phone case.
...
|------------|---------------------------|
| 0.1.29+20  | P5.1 Trip finalize fix (client-side). До этого `_syncTrips` использовал `t.id > _cursorTrip` для gating push'а — это работало ровно один раз на trip при его insert'е (с `endedAt=NULL` + все aggregates NULL); после `endTrip()` в Drift trip row обновлялся, но cursor уже сдвинут → finalize-данные на сервер никогда не уходили. Друг 2 подтвердил симптом на проде: trip id=1 от утра 2026-05-25 лежал в Postgres с ended_at=NULL. Этот патч идёт в паре с серверным UPSERT (commit Друга 2 58d805d, §P5.2 ROADMAP); UPSERT уже задеплоен и зелёный на 54/54 тестах. CommandStatus literal alias `finished` принят на сервере (Pydantic union) — backward-compat для старого клиентского кода, cleanup на стороне клиента можно сделать в отдельном патче. Изменения в CloudSyncService: новый SharedPrefs key `cloud_sync_pushed_trip_ids` (JSON int list), новое in-memory поле `Set<int> _pushedTripIds`. Push gating: `where t.endedAt != null && !_pushedTripIds.contains(t.id)` → POST → `_pushedTripIds.addAll(batch.ids)` → persist. Server-side UPSERT с unique constraint (device_id, client_trip_id) absorbs accidental re-push: получаем `{received:N, inserted:0, duplicates:N}` — это **не** error, это успешный merge. Открытые trips (endedAt=null) intentionally not pushed — owner не запрашивал live monitoring во время drive'а. **No migration logic**: на first launch +20 после upgrade с +17/+18/+19 `_pushedTripIds` пустой, следующий syncOnce пушит ВСЕ closed trips заново. UPSERT absorbs all duplicates безопасно. Это явный design choice вместо ранее планировавшейся миграции (которая seed'ила бы pushed set из существующих closed trips ≤ _cursorTrip) — миграция имела subtle bug: trip который был closed в Drift локально но push'нут +19 до finalize (т.е. с NULL aggregates) — попал бы в seed как pushed → re-push не произошёл бы → server-row остался бы NULL forever. Чистый replay безопаснее. `_cursorTrip` retained для stats (max id seen), но больше не gating; `forceFullResync()` дополнительно сбрасывает `_pushedTripIds` (это и есть основная семантика "replay everything"). Restore path: closed trips добавляются в set (они уже на сервере), open trips остаются вне (они могут быть half-finalized что мы хотим re-push'нуть когда finalize произойдёт локально). `_recomputeStats` для trips переписан с `maxTrip - _cursorTrip` на `count(closed && not pushed)` — точно отражает actionable pending. Snapshots/sweeps/livelogs не тронуты (append-only, cursor стратегия валидна). End-to-end validation (planned): head unit на +19 → uninstall +19 / install +20 (Toyota DiLink не позволяет in-place update, см. TL;DR п.3) → Drift и SharedPrefs стёрты → "Restore from cloud" с client_token из Bitwarden → trip id=1 + 19 snapshots возвращаются в локальный Drift в том виде в котором они были на сервере (trip id=1 приходит с ended_at=null потому что сервер тоже имеет open trip — финализированная версия из +19 Drift'а была локальная и стёрта uninstall'ом). Restore success path добавляет в pushed set только closed trips (endedAt != null) — trip id=1 НЕ попадает в set. **Trip id=1 на сервере останется ended_at=NULL forever**: финализационные данные были только в локальном Drift'е +19 и потеряны при uninstall'е, +20 их не пересоздаст из restored snapshots. Это accepted loss первого production trip'а; админу/Другу 2 можно DELETE'ом подчистить ряд. Реальная валидация UPSERT-flow'а пойдёт на **следующих** drives после +20: новый trip id=N (с локальным id, сгенерённым Drift'ом после restore) → drive close → endTrip() → следующий syncOnce пушит → server UPSERT обрабатывает как `{received:1, inserted:1, duplicates:0}` для первого push, затем как `{received:1, inserted:0, duplicates:1}` при последующих push'ах если случится (но dirty-tracking их не пустит без новых изменений). Touched: cloud_sync_service.dart (~70 LOC net add). pubspec + _kDiagVersion + _readAppVersion → 0.1.29+20. Database.dart не тронут (no schema change, использует existing `getAllTrips()`). |
| 0.1.29+19  | Build fix к +18: kernel_snapshot упал с `The operator '&' isn't defined for the class 'Expression<bool>'` в trips dedup внутри `CloudSyncService.startRestore`. Причина: я писал `cond & t.distanceKm.equals(...)` строя Expression вручную, но `&` для `Expression<bool>` определён через extension в `package:drift/drift.dart`, а я импортирую только `show Value`. Fix: переписал на множественные `.where()` вызовы (Drift ANDит их автоматически) — это и идиоматичнее (см. паттерн в `getSamplesForTrip` в database.dart), и не требует расширять import. Никакой функциональной разницы — query производит идентичный SQL. **Process learning**: check_repo.py делает только синтаксический brace-balance и regex'ы; тип-системные ошибки Dart'а ловятся только kernel_snapshot'ом на CI. Это known limitation, отмечена в HANDOFF "type errors NOT checkable in this container" — поэтому каждый patch должен ждать первого CI green'а перед тем как считаться доделанным. Touched: cloud_sync_service.dart (1 query rewritten + bump readAppVersion). pubspec + _kDiagVersion → 0.1.29+19. |
| 0.1.29+18  | P1.5 Cloud restore (strategy A+restore per HANDOFF; head-unit reinstall is the primary use case — Toyota DiLink не поддерживает in-place apk update, только uninstall+reinstall, что стирает Drift+secureStorage+SharedPrefs). CloudSyncService получил три новых публичных метода: `probeRestoreToken(t)` — валидация candidate client_token через `GET /v1/data/trips?limit=1` (возвращает device_id из префикса token'а); `startRestore({oldClientToken})` — fire-and-forget pull: swap token + secureStorage write → отключение periodic push на время → `GET /v1/data/trips` (limit=100) и `GET /v1/data/snapshots` (limit=200) с pagination через `next_cursor`, dedup `(started_at, distance_km)` для trips и `captured_at` для snapshots, билд `Map<int,int> serverClientTripId → newLocalId` для rewrite snapshot.tripId, finally — `_cursorTrip/_cursorSnapshot = max(local id)` (предотвращает re-push restored rows под restored identity), `_enabled=true` на success (чтобы periodic sync стартовал после restore-from-disconnected); `cancelRestore()` — флаг для loop, проверяется между pages. Новый `_getJson(path, query)` helper (~70 LOC, focus: 401 → _Transient/AuthException, 408/429/5xx → backoff из _retryBackoff). Новые enum `CloudRestoreStatus` (idle/preflight/fetching/done/cancelled/error) + value class `CloudRestoreProgress`. SharedPrefs: `_kLastRestoreAt`/`_kLastRestoreError` (status non-persistent — на app start всегда idle). Также добавлен getter `clientTokenForBackup` (единственное публичное чтение `_clientToken` — для UI бэкапа в password manager; sync paths используют `_clientToken` напрямую). Settings UI: новая кнопка "Restore from cloud" появляется **в двух местах** — (1) в disconnected-state как альтернатива Setup, рядом с "Set up cloud backup" (это ключевой head-unit reinstall path — нужен только один токен от Друга 2, без round-trip за setup_token); (2) в registered-state в Wrap'е кнопок между Force resync и Disconnect. 3-фазный диалог: token → probe loader → confirmation с warning про local-id collision → progress dialog с `Consumer<CloudSyncService>` + Cancel/Close. После успеха в Settings — `BridgeDiagService.refreshTokenFromSharedStorage()` (Plane A разделяет тот же secureStorage token; та же логика что и после Setup). Также в registered-state Wrap добавлена кнопка "Backup token" — `_showBackupTokenDialog` с reveal-on-tap (default hidden как bullets), Copy via Clipboard.setData + snackbar, warning text "Server cannot recover it (sha256-hashed)". Owner-self-service: после первого Restore (когда у Друга 2 будет endpoint POST /v1/admin/devices/{id}/rotate-token, Phase 5 P5.X) owner один раз нажмёт Backup → сохранит в Bitwarden → все следующие реинсталлы self-contained без Друга 2 в loop'е. Inline restore status card отображается и в disconnected-, и в registered-секции Cloud backup body. Sync / Force resync / Disconnect / Backup token disabled когда isRestoring. Scope этой итерации: только trips + snapshots; sweeps/livelogs — отдельный патч если понадобится. Touched: cloud_sync_service.dart (+~450 LOC), settings.dart (+~390 LOC, добавлены `import 'dart:async'` для unawaited + `flutter/services` для Clipboard). Protected files (database.dart/connection.dart/elm327_ble.dart) **не тронуты** — restore делает raw `_db.into(_db.trips)` / `_db.select(_db.snapshots)` запросы через standard Drift external API, добавлен только `import 'package:drift/drift.dart' show Value`. cloud_sync `_readAppVersion` → 0.1.29+18. Друг 2 подтвердил response shape: `{items:[...], next_cursor: str | null}`, sort `id DESC`, опаковый numeric cursor — мой код bit-for-bit совместим без правок. **NOTE: build упал в kernel_snapshot на `&` для Expression<bool>; fix в +19.** |
| 0.1.29+17  | P1.1 Турн B — BLE command handlers в BridgeDiagService. Конструктор стал `BridgeDiagService({required ConnectionService svc})` (constructor injection, option a из HANDOFF). `_handleCommand` switch'ит по kind: `bleStartSweep` → `ConnectionService.runSweep()` fire-and-forget (возвращает `{started:true, kind:"sweep"}` немедленно после валидации; sweep идёт в фоне минуты-часы); `bleStartLiveLog` → `runLiveLog()` fire-and-forget (did_list = JSON objects `{tx_ecu,rx_ecu,did}`, uppercase hex; period_ms игнорируется → `ignored_args:["period_ms"]`); `bleStopActiveOperation` → `cancelSweep()+cancelLiveLog()`, idempotent, возвращает `{stopped:["sweep"|"livelog"|[]]}`. error_kind whitelist (согласован с Другом 2): busy / not_connected / parse / validation / unsupported / unknown. connection.dart: 1 additive getter `isBleConnected` (adapterAddress не годится — set при первом pairing'е, never cleared). Smoke-test командами #3-8 прошёл 5/5. cloud_sync `_readAppVersion` → 0.1.29+17. |
