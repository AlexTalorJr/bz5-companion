...
def main(path):
    ...
    # v0.1.97+196 — ЧЕСТНОЕ «ДАННЫХ НЕТ» ВМЕСТО ПАДЕНИЯ.
    #
    # Экспорт 05.08 уронил зеркало на TypeError: сырьё поездки #155 в базе
    # не пережило переустановку — облако несёт поездки и снапшоты, а
    # строки журнала HAL нет, они локальные. Инструмент проверки обязан
    # отличать «предмет проверки отсутствует» от «проверка провалилась»:
    # первое требует другого файла, второе — правки кода.
    if not window:
        print('  [SKIP] строк журнала поездки #%d в этой базе нет.' % TRIP)
        print('         Зеркалу нечего проигрывать. Это не отказ проверки:')
        print('         сырьё HAL живёт только локально и не переживает')
        print('         переустановку — облако его не несёт.')
        print('         Нужен экспорт, снятый ДО переустановки.')
        print('=' * 60)
        print('MIRROR SKIP — нет данных')
        return 0
